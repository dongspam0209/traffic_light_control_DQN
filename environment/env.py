...
class SumoEnv(gym.Env):
    ...
    def set_sumo(selt, gui, sumocfg_file_name, max_steps):
        if 'SUMO_HOME' in os.environ:
            tools = os.path.join(os.environ['SUMO_HOME'], 'tools')
            sys.path.append(tools)
        else:
    ...
    # setting the cmd mode or the visual mode    
        ...
        sumo_cmd = [sumolib.checkBinary(self.sumoBinary), '-c', os.path.join('intersection', sumocfg_file_name),  "--no-step-log", "true", "--waiting-time-memory", str(max_steps)]
        return sumo_cmd
