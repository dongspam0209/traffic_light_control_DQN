...
class DqnAgent: 
    def __init__(self, mode: str, replay, target_update: int, gamma: float, use_sgd: bool, eps_start: float, eps_end: float, eps_decay: int, input_dim: int, output_dim: int, batch_size: int, network_file: str = ''
        ...
        self.policy_net = network.DQN(input_dim, output_dim).to(device)
        self.target_net = network.DQN(input_dim, output_dim).to(device)
        self.policy_net_copy = network.DQN(input_dim, output_dim).to(device)
        if network_file:
